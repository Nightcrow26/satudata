{{-- resources/views/components/public/walidata/filter-panel.blade.php --}}
@props([
// Opsi
'sortOptions' => ['recent' => 'Terbaru', 'oldest' => 'Terlama', 'popular' => 'Terpopuler', 'name' => 'Nama A-Z'],
'aspekOptions' => [], // ['tek'=>'Teknologi', ...]
'instansiOptions' => [], // ['dinkes'=>'Dinas Kesehatan', ...]
'bidangOptions' => [], // ['kesehatan'=>'Kesehatan', ...]
'indikatorOptions' => [], // ['angka-kematian'=>'Angka Kematian', ...]
'tahunOptions' => [], // [2023, 2022, ...] - TIDAK DIGUNAKAN LAGI
'showSearch' => false, // set true jika mau tampilkan search di panel
'searchPlaceholder' => 'Cari data yang anda butuhkan',

// Nama properti Livewire untuk binding
'qModel' => 'search',
'sortModel' => 'sort',
'aspekModel' => 'selectedAspek',
'instansiModel' => 'selectedInstansi',
'bidangModel' => 'selectedBidang',
'indikatorModel' => 'selectedIndikator',
'tahunModel' => 'selectedTahun',

// Nilai terpilih untuk SSR (opsional tapi disarankan)
'selected' => [
'q' => '',
'sort' => null,
'selectedAspek' => [],
...
'selectedBidang' => [],
'selectedIndikator' => [],
'selectedTahun' => [], // TIDAK DIGUNAKAN LAGI
],
...
<aside {{ $attributes->merge([
    ...
    {{-- Aspek (checkbox, dengan "Selengkapnya") --}}
    <fieldset>
        ...
        @php
        $aspekAll = collect($aspekOptions);
        $aspekHead = $aspekAll->slice(0, $previewCount);
        $aspekTail = $aspekAll->slice($previewCount);
        $aspekSelected = collect(data_get($selected, 'selectedAspek', []))->filter()->values()->all();
        @endphp

        <div class="mt-2 space-y-2">
            @foreach($aspekHead as $val => $label)
            <label class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                <span class="flex-none pt-0.5">
                    <input type="checkbox"
                        class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-teal-600 focus:ring-teal-600 dark:focus:ring-teal-400"
                        value="{{ $val }}" wire:model.live="{{ $aspekModel }}" @checked(in_array($val, $aspekSelected,
                        true))>
                </span>
                <span class="flex-1 leading-snug break-words">{{ $label }}</span>
            </label>
        ...
        @if($aspekTail->isNotEmpty())
        <details class="mt-2">
            <summary class="list-none inline-flex items-center gap-1 text-[13px] font-medium
                           text-teal-700 hover:text-teal-800
                           dark:text-teal-400 dark:hover:text-teal-300 cursor-pointer">
                Selengkapnya
    ...
    {{-- Instansi (checkbox) --}}
    <fieldset>
        <legend class="text-sm font-semibold text-gray-800 dark:text-white">Instansi</legend>
        @php
        $instAll = collect($instansiOptions);
        $instHead = $instAll->slice(0, $previewCount);
        $instTail = $instAll->slice($previewCount);
        $instSelected = collect(data_get($selected, 'selectedInstansi', []))->filter()->values()->all();
        @endphp
    ...
    {{-- Bidang (checkbox) --}}
    <fieldset>
        <legend class="text-sm font-semibold text-gray-800 dark:text-white">Bidang</legend>
        @php
        $bidangAll = collect($bidangOptions);
        $bidangHead = $bidangAll->slice(0, $previewCount);
        $bidangTail = $bidangAll->slice($previewCount);
        $bidangSelected = collect(data_get($selected, 'selectedBidang', []))->filter()->values()->all();
        @endphp

        <div class="mt-2 space-y-2">
            @foreach($bidangHead as $val => $label)
            <label class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                <span class="flex-none pt-0.5">
                    <input type="checkbox"
                        class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-teal-600 focus:ring-teal-600 dark:focus:ring-teal-400"
                        value="{{ $val }}" wire:model.live="{{ $bidangModel }}" @checked(in_array($val, $bidangSelected,
                        true))>
                </span>
                <span class="flex-1 leading-snug break-words">{{ $label }}</span>
            </label>
        ...
        <details class="mt-2">
            ...
            <div class="mt-2 max-h-48 overflow-y-auto pr-1 space-y-2">
                @foreach($bidangTail as $val => $label)
                <label
                    class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                    <span class="flex-none pt-0.5">
                        <input type="checkbox"
                            class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-teal-600 focus:ring-teal-600 dark:focus:ring-teal-400"
                            value="{{ $val }}" wire:model.live="{{ $bidangModel }}" @checked(in_array($val,
                            $bidangSelected, true))>
                    </span>
                    <span class="flex-1 leading-snug break-words">{{ $label }}</span>
                </label>
    ...
    {{-- Indikator (checkbox) --}}
    <fieldset>
        <legend class="text-sm font-semibold text-gray-800 dark:text-white">Indikator</legend>
        @php
        $indikatorAll = collect($indikatorOptions);
        $indikatorHead = $indikatorAll->slice(0, $previewCount);
        $indikatorTail = $indikatorAll->slice($previewCount);
        $indikatorSelected = collect(data_get($selected, 'selectedIndikator', []))->filter()->values()->all();
        @endphp

        <div class="mt-2 space-y-2">
            @foreach($indikatorHead as $val => $label)
            <label class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                <span class="flex-none pt-0.5">
                    <input type="checkbox"
                        class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-teal-600 focus:ring-teal-600 dark:focus:ring-teal-400"
                        value="{{ $val }}" wire:model.live="{{ $indikatorModel }}" @checked(in_array($val, $indikatorSelected,
                        true))>
                </span>
                <span class="flex-1 leading-snug break-words" title="{{ $label }}">{{ Str::limit($label, 30) }}</span>
            </label>
            @endforeach
        </div>

        @if($indikatorTail->isNotEmpty())
        <details class="mt-2">
            <summary class="list-none inline-flex items-center gap-1 text-[13px] font-medium
                           text-teal-700 hover:text-teal-800
                           dark:text-teal-400 dark:hover:text-teal-300 cursor-pointer">
            ...
            <div class="mt-2 max-h-48 overflow-y-auto pr-1 space-y-2">
                @foreach($indikatorTail as $val => $label)
                <label
                    class="flex items-start gap-3 text-sm text-gray-700 dark:text-gray-200 cursor-pointer select-none">
                    <span class="flex-none pt-0.5">
                        <input type="checkbox"
                            class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-teal-600 focus:ring-teal-600 dark:focus:ring-teal-400"
                            value="{{ $val }}" wire:model.live="{{ $indikatorModel }}" @checked(in_array($val,
                            $indikatorSelected, true))>
                    </span>
                    <span class="flex-1 leading-snug break-words" title="{{ $label }}">{{ Str::limit($label, 30) }}</span>
                </label>
                @endforeach
