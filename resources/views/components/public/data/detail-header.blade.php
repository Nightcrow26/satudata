{{-- resources/views/components/public/data/detail-header.blade.php --}}
@props(['dataset' => null, 'class' => ''])

@php
$d = is_array($dataset) ? (object) $dataset : ($dataset ?? (object) []);
$title = $d->title ?? 'Judul Dataset Placeholder';
$summary = $d->summary ?? 'Deskripsi placeholder lorem ipsum dolor sit amet. Teks ini akan diganti dengan deskripsi
singkat tentang dataset ketika data riil tersedia.';

$publishedAt = $d->published_at ?? '17 Januari 2025';
$views = $d->views ?? 112;
@endphp
...
<div {{ $attributes->merge([
    ...
    <div class="grid grid-cols-1 md:grid-cols-[1fr_auto] md:items-start gap-4 sm:gap-6 md:gap-8">
        ...
        <div>
            ...
            <div class="mt-4 text-base text-slate-600 dark:text-gray-300 content-html trix-content">
                {!! $d->deskripsi !!}
            </div>
        </div>
        ...
        <div class="flex flex-col md:items-end gap-3 md:gap-4">
            <div class="flex flex-wrap items-center gap-x-6 gap-y-3 text-[15px] text-slate-600 dark:text-gray-300">
                <span class="inline-flex items-center gap-2 text-teal-700 dark:text-teal-400">
                    ...
                    <span>{{ $d->created_at->translatedFormat('d F Y') }}</span>
                </span>
            ...
            <div class="-mx-1.5 flex items-center gap-1.5 md:gap-3">
                ...
                {{-- Download PDF --}}
                @livewire('admin.download-pdf', ['dataset' => $d])
                ...
                <a type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium text-green-600 dark:text-green-400 bg-white dark:!bg-gray-800 border border-green-300 dark:border-green-600 rounded-md hover:bg-green-50 dark:hover:bg-red-900/20 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                 aria-label="Unduh CSV"
                               href="{{ route('public.data.download', $d) }}">
