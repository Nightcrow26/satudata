{{-- resources/views/livewire/public/agencies/index.blade.php --}}
<div>
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-14">
        ...
        <div x-data="searchShortcut()" @keydown.window="handleKeydown($event)" class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-6 p-4
                   rounded-xl border bg-gray-50/50 border-gray-100
                   dark:bg-gray-900/40 dark:border-gray-700/60
            ...
            <div class="relative w-full sm:w-80">
                <label for="q" class="sr-only">Cari instansi</label>
                <div class="relative">
                    <input id="q" x-ref="searchInput" type="search" wire:model.live.debounce.300ms="q"
                        placeholder="Cari nama instansi..." aria-describedby="search-shortcut" class="h-11 w-full rounded-lg border-0 pl-12 pr-10 text-sm
                               ...
                               dark:placeholder-gray-400 dark:focus:bg-gray-900
            ...
            <div x-data="{
                    ...
                    open: false,
                    getSortLabel() { return this.sort === 'recent' ? 'Terbaru' : 'Nama (A-Z)'; },
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="py-1">
                        <button @click="sort = 'recent'; open = false" type="button" class="w-full flex items-center gap-3 px-4 py-2 text-sm text-left
                                ...
                                'bg-teal-50 text-teal-700 font-medium dark:bg-teal-900/40 dark:text-teal-300': sort === 'recent'
                            }">
                        ...
                        <button @click="sort = 'name'; open = false" type="button" class="w-full flex items-center gap-3 px-4 py-2 text-sm text-left
                                   hover:bg-gray-50 text-gray-700
                                   dark:text-gray-200 dark:hover:bg-gray-700
                                   transition-colors duration-150" :class="{
                                'bg-teal-50 text-teal-700 font-medium dark:bg-teal-900/40 dark:text-teal-300': sort === 'name'
                            }">
                            <svg class="h-4 w-4 flex-shrink-0"
                                :class="{ 'text-teal-600 dark:text-teal-300': sort === 'name', 'text-gray-400 dark:text-gray-400': sort !== 'name' }"
                                viewBox="0 0 20 20" fill="currentColor">
                                <path
                                    d="M2 5a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 012 5zm0 5a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 012 10zm0 5a.75.75 0 01.75-.75h4.5a.75.75 0 010 1.5h-4.5A.75.75 0 012 15zm8.5-10a.75.75 0 01.75-.75h6a.75.75.75 0 010 1.5h-6a.75.75 0 01-.75-.75zm0 5a.75.75 0 01.75-.75h6a.75.75 0 010 1.5h-6a.75.75 0 01-.75-.75zm0 5a.75.75 0 01.75-.75h6a.75.75 0 010 1.5h-6a.75.75 0 01-.75-.75z" />
                            </svg>
                            <span>Nama (A–Z)</span>
                            <svg x-show="sort === 'name'" class="h-4 w-4 ml-auto text-teal-600 dark:text-teal-300"
                                viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd"
                                    d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z"
                                    clip-rule="evenodd" />
        ...
        {{-- Grid Kartu Instansi (responsive & accessibility) --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8" role="list"
            aria-label="Daftar instansi">
            @forelse($agencies as $agency)
            <article role="listitem">
                <a href="{{ $agency['href'] }}" class="group block rounded-2xl border bg-white border-gray-200 shadow-sm hover:shadow-md
                           focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-600 focus-visible:ring-offset-2
                           dark:bg-gray-800 dark:border-gray-700 dark:focus-visible:ring-offset-gray-900
                           transition-colors duration-200" aria-label="Lihat detail {{ $agency['name'] }}"
                    wire:navigate>
                    <x-public.agencies.card :name="$agency['name']" :logo="$agency['logo']"
                        :dataset="$agency['dataset']" :pubs="$agency['pubs']" :walidata="$agency['walidata']" :updated="$agency['updated']"
                        :views="$agency['views']" />
                </a>
            </article>
            @empty
            <div class="sm:col-span-2 lg:col-span-3">
                <div class="rounded-2xl border bg-white border-gray-200 shadow-sm p-8 text-center text-gray-600
                           dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400
                           transition-colors duration-200" role="status" aria-live="polite">
                    ...
                    <p class="text-base font-medium text-gray-900 dark:text-gray-100 mb-1">Belum ada instansi</p>
                    <p class="text-sm text-gray-600 dark:text-gray-300">Belum ada instansi yang dapat ditampilkan untuk
                        kriteria pencarian ini.</p>
                </div>
            </div>
            @endforelse
        </div>

        {{-- Footer: pagination (info count sudah include di pagination template) --}}
        <div class="mt-6 sm:mt-8">
            {{ $agencies->onEachSide(1)->links('vendor.pagination.agencies') }}
        </div>
    </section>
</div>
