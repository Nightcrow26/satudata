  ...
  <div class="bg-white dark:!bg-gray-800 shadow-sm rounded-lg">
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
      <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
          ...
          <h3 class="mt-3 text-lg font-medium text-gray-900 dark:text-white">Pengguna</h3>
   ...
   <div class="p-6">
          ...
          <tbody class="bg-white dark:!bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            @foreach($users as $idx => $user)
              <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $users->firstItem() + $idx }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $user->name }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ Str::limit($user->email, 30) }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-900 dark:text-gray-100">{{ $user->nik}}</td>  
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ Str::limit($user->skpd?->nama, 30) }}</td>
                @php
                    // Map role => Tailwind badge classes
                    $badgeClasses = [
                        'admin'       => 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300',
                        'verifikator' => 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300',
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center">
                    @foreach($user->getRoleNames() as $r)
                        @php
                            $variant = $badgeClasses[strtolower($r)] ?? 'bg-gray-100 text-gray-800 dark:!bg-gray-800 dark:text-gray-300';
                        @endphp
                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {{ $variant }}">{{ ucfirst($r) }}</span>
                    @endforeach
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-center">
    ...
    <div wire:ignore.self wire:key="user-modal"
            ...
            class="bg-white dark:!bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-600 w-full max-w-lg opacity-100">
                <form wire:submit.prevent="saveUser">
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4" wire:key="user-content-{{ $user_id ?: 'new' }}">
                            <div>
                            ...
                            <div>
                                <x-forms.file-input 
                                    ...
                                    name="sk_penunjukan"
                                    wire:model="sk_penunjukan"
                                    accept="application/pdf"
                                    :existingFile="$current_sk_penunjukan ? basename($current_sk_penunjukan) : null"
                                    :existingFileUrl="$editingUser?->sk_penunjukan ? Storage::disk('s3')->temporaryUrl($editingUser->sk_penunjukan, now()->addMinutes(15)) : null"
                                />
    ...
    <template x-teleport="body">
    <div wire:ignore.self wire:key="user-delete-modal"
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    Apakah Anda yakin ingin menghapus user <strong class="text-gray-900 dark:text-white">{{ $name }}</strong>?
                                </p>
                            </div>
                        </div>
                    </div>
