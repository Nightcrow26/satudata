...
@auth
  ...
  <div class="chat-body rounded-lg" x-ref="scrollArea" x-show="hasAnyChat">
    ...
    <template x-if="streaming && (pending.answer.length > 0 || requestInProgress)">
      <div class="chat-msg ai">
        <div class="chat-bubble ai">
          <div class="role-tag loading">ASISTEN</div>
          <div class="content" x-text="pending.answer || 'Menyiapkan jawaban...'"></div>
  ...
  <form wire:submit.prevent="startStream" class="chat-input w-full bg-white dark:!bg-gray-900">
    <div class="flex w-full gap-2 items-stretch rounded-lg">
      <input type="text" class="flex-1 min-w-0 px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:!bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed" 
             placeholder="Ketik pertanyaan…"
             wire:model.defer="input"
             :disabled="streaming || @js($isStreaming)"
             x-bind:class="{ 'opacity-50': streaming || @js($isStreaming) }">
      <button class="px-4 py-3 bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" 
              ...
              :disabled="streaming || @js($isStreaming)"
              x-bind:class="{ 'opacity-50': streaming || @js($isStreaming) }">
        <span x-show="!streaming && !@js($isStreaming)">
          <i class="bi bi-send"></i>
        </span>
        <span x-show="streaming || @js($isStreaming)" class="flex items-center">
