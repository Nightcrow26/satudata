<div>
    ...
    <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
      <div class="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
        ...
        <div class="flex items-center flex-wrap gap-2">
          <h3 class="text-lg mt-3 font-medium text-gray-900 dark:text-white">Bidang Urusan</h3>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:!bg-gray-800">
            <tr>
              ...
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Kode Bidang</th>
              <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Uraian Bidang</th>
          ...
          <tbody class="bg-white dark:!bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            @foreach($bidangs as $idx => $bidang)
              <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $bidangs->firstItem() + $idx }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100">{{ $bidang->kode_bidang }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{{ $bidang->uraian_bidang }}</td>
  ...
  <template x-teleport="body">
    <div wire:ignore.self wire:key="bidang-modal"
          ...
          <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">
                {{ $bidang_id ? 'Edit Bidang' : 'Tambah Bidang' }}
              </h3>
              <button type="button"
            ...
            <div class="space-y-4" wire:key="bidang-content-{{ $bidang_id ?: 'new' }}">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Kode Bidang</label>
                <input type="text" 
                       class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:!bg-gray-800 dark:text-white dark:focus:ring-indigo-400 dark:focus:border-indigo-400 {{ $errors->has('kode_bidang') ? 'border-red-300 dark:border-red-600' : '' }}" 
                       wire:model.defer="kode_bidang" 
                       placeholder="Masukkan kode bidang">
                @error('kode_bidang') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
              </div>
              
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Uraian Bidang</label>
                <input type="text" 
                       class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:!bg-gray-800 dark:text-white dark:focus:ring-indigo-400 dark:focus:border-indigo-400 {{ $errors->has('uraian_bidang') ? 'border-red-300 dark:border-red-600' : '' }}" 
                       wire:model.defer="uraian_bidang" 
                       placeholder="Masukkan uraian bidang">
                @error('uraian_bidang') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
              </div>
            </div>
          </div>
  ...
  <template x-teleport="body">
    <div wire:ignore.self wire:key="bidang-delete-modal"
            ...
            <div x-show="show"
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 dark:text-gray-300">
                                    Yakin ingin menghapus bidang <strong>{{ $nama }}</strong>?
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-gray-50 dark:!bg-gray-800 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse rounded-b-lg">
                    <button type="button"
                            ...
                            wire:click="deleteBidangConfirmed"
                            @click="show = false">
                        Hapus
                    </button>
                    <button type="button"
